...
from ..utils import _default_2d_coupling_function


class Conv2dRealNVP(ConvNVP):
    '''
    ...
    def __init__(self, channels, kernel_size, w=4, activation_fn=nn.ReLU, s=None, t=None, mask=None, clip=True, clip_n=1.0):
        super(Conv2dRealNVP, self).__init__(channels, kernel_size, w=w, activation_fn=activation_fn, s=s, t=t, mask=mask, clip=clip, clip_n=clip_n)
        if s is None:
    ...
    def forward(self, x, log_p0=0, log_det_J=0):
        y = super(Conv2dRealNVP, self).forward(x)
        if self.compute_p:
